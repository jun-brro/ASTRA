...
data_path = "./datasets/adv_image_toxic/minigpt"
...
toxic_prompts = pd.read_csv('./datasets/{}_rtp_prompts.csv'.format(args.eval))['prompts']
...
for index, toxic_prompt in tqdm(enumerate(toxic_prompts)):
    ...
    with torch.no_grad(), torch.amp.autocast('cuda'):    
        ...
        for i, (steer_type, alpha) in enumerate(zip(steer_types, alphas)):
            ...
            steered_text, output_tokens = generator.generate(prompt_wrap)
...
data.to_csv('./results/responses/minigpt/toxic/detoxify_minigpt_{}_{}_{}_alpha_{}.csv'.format(
...
for i in range(num_steer_op):
    ...
    print(args.feat_type, steer_types[i], alphas[i], "Toxicity_score: {:.2f}% | Any: {:.2f}% | Identity Attack: {:.2f}% | obscene: {:.2f}% | Severe Toxicity: {:.2f}% | Insult: {:.2f}% | Threat: {:.2f}% | Toxicity: {:.2f}%".format(
