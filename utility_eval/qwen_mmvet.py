...
os.makedirs('./results/utility/qwen/mmvet/', exist_ok=True)
...
for index, (key, line) in tqdm(enumerate(questions.items()), total=len(questions)):
    ...
    with torch.no_grad(), torch.amp.autocast('cuda'):    
        ...
        for i, (steer_type, alpha) in enumerate(zip(steer_types, alphas)):
            ...
            generate_ids = model.generate(**inputs, do_sample=True, max_length=2048, temperature=0.2, top_p=0.9,)
...
data.to_csv('./results/utility/qwen/mmvet/qwen_{}_{}_{}_{}_alpha_{}.csv'.format(
