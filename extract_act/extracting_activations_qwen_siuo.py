...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Extract QWEN SIUO Activations")
...
def process_category_activations(category, task_type, model, processor, tokenizer):
    """Process activations for a single category"""
    print(f"\n{'='*60}")
    print(f"Processing activations for category: {category} (task: {task_type})")
    print(f"{'='*60}")
    
    # Load datasets for this category
    data_img_path = f"./results/adv_img_attr/qwen_siuo/{task_type}/{category}"
    queries_path = f'./results/queries/qwen_siuo_{task_type}_{category}.csv'
    
    # Check if the required files exist
    if not os.path.exists(queries_path):
        print(f"Error: Query file not found: {queries_path}")
        print("Please run extract_qwen_siuo_attr.py first!")
        return None
    ...
    save_path = "./activations/qwen/siuo"
    os.makedirs(save_path, exist_ok=True)
    
    output_file = os.path.join(save_path, f"siuo_diff_attr_activations_{variable_element}_{task_type}_{category}.pt")
...
for category in categories_to_process:
    try:
        processed = process_category_activations(category, args.task_type, model, processor, tokenizer)
